...
import type { SearchParams, SearchResult } from "./public-property.types.js";
...
export const searchProperties = async (params: SearchParams): Promise<SearchResult> => {
  ...
  const rows = await prisma.$queryRawUnsafe<any[]>(query, checkIn, checkOut, search, category, city, limit, offset);
