...
export const getRoomCalendarPrices = async (roomId: string, monthStr: string) => {
  ...
  if (parts.length !== 2) throw new Error("Format bulan tidak valid. Gunakan YYYY-MM.");

  const year = parseInt(parts[0]!, 10);
  const month = parseInt(parts[1]!, 10) - 1;
  if (isNaN(year) || isNaN(month)) throw new Error("Format bulan tidak valid. Gunakan YYYY-MM.");
