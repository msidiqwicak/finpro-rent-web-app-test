...
export const login = async (email: string, password: string, requestedRole: 'USER' | 'TENANT') => {
...
export const requestPasswordReset = async (email: string) => {
  ...
  const actualRole = await prisma.tenant.findUnique({ where: { user_id: user.id } }) ? 'TENANT' : 'USER';
  const token = generateResetToken({ id: user.id, email: user.email, role: actualRole });
