...
export const getPropertyDetails = async (
  ...
  const prop = await prisma.property.findUnique({
    ...
    include: {
      ...
      tenant: {
        select: {
          id: true,
          // Ambil juga name & avatar dari tabel users milik tenant
      ...
      review: {
        include: {
          users: {
            select: { name: true, avatar_url: true }, // Ambil info tamu yang mereview
          },
        },
        orderBy: {
          created_at: "desc", // Ulasan terbaru di atas
        },
      },
      // 👆 ========================================= 👆
    },
  ...
  const roomTypesWithPrice = prop.room_type.map((rt) => ({
    ...
    adjusted_price: calcAdjustedPrice(
      rt.price_per_night,
      rt.price_modifier,
      target,
    ),
...
export const getRoomCalendarPrices = async (
  roomId: string,
  monthStr: string,
) => {
