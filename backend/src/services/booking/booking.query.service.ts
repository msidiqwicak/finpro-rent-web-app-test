...
export const getAllBookings = async (userId: string, search?: string, date?: string) => {
  const whereClause: any = { user_id: userId };
  if (date && date.trim() !== "") whereClause.check_in = new Date(date);
...
export const getBookingsByTenant = async (tenantId: string, search?: string, status?: string) => {
  const whereClause: any = { room_unit: { room_type: { property: { tenant_id: tenantId } } } };
