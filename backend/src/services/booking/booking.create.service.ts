...
const validateDates = (checkIn: Date, checkOut: Date) => {
  ...
  if (checkIn < today) throw new Error("Tidak dapat memesan untuk tanggal di masa lalu.");
  if (checkOut <= checkIn) throw new Error("Check-out harus setelah check-in.");
};

const checkBlockDates = async (roomTypeId: string, checkIn: Date, checkOut: Date) => {
...
const autoAssignUnit = async (roomTypeId: string, checkIn: Date, checkOut: Date) => {
  ...
  if (!unit) throw new Error("Maaf, tipe kamar ini sudah penuh untuk tanggal tersebut.");
  return unit.id;
};

const calculateTotalPrice = async (roomTypeId: string, checkIn: Date, checkOut: Date) => {
...
export const createBookingProcess = async (userId: string, roomTypeId: string, checkIn: Date, checkOut: Date) => {
