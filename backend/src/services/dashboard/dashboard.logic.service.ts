export const calculateGrowth = (thisMonth: number, lastMonth: number) => {
...
export const processRevenue = (bookings: any[], now: Date) => {
  let total = 0, thisMonth = 0, lastMonth = 0;
  const propRevs: Record<string, any> = {};
  ...
  bookings.forEach((b) => {
    ...
    total += rev;
    if (bDate.getMonth() === curM && bDate.getFullYear() === curY) thisMonth += rev;
    else if (bDate.getMonth() === lastM.getMonth() && bDate.getFullYear() === lastM.getFullYear()) lastMonth += rev;

    const mDiff = (curY - bDate.getFullYear()) * 12 + (curM - bDate.getMonth());
    if (mDiff >= 0 && mDiff < 6) monthly[5 - mDiff] += rev;

    const prop = b.room_unit?.room_type?.property;
    if (prop) {
      if (!propRevs[prop.id]) propRevs[prop.id] = { property: prop, revenue: 0, count: 0 };
...
export const getChartData = (monthly: number[], now: Date) => {
...
export const getTopProperty = (propRevs: Record<string, any>) => {
  const sorted = Object.values(propRevs).sort((a: any, b: any) => b.revenue - a.revenue);
