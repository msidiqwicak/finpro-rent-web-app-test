...
export const sendEmailChangeVerificationEmail = async (to, token) => {
...
export const sendConfirmationEmail = async (to, bookingData) => {
    const propertyName = bookingData.room_unit?.room_type?.property?.name || "Properti Kami";
    const checkInDate = new Date(bookingData.check_in).toLocaleDateString("id-ID", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
    const checkOutDate = new Date(bookingData.check_out).toLocaleDateString("id-ID", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
    if (!transporter) {
        console.log("\n📬 ========================================================");
        console.log("📧 [MOCK EMAIL - Booking Confirmation]");
        console.log(`Tujuan: ${to}`);
        console.log(`Subjek: Pembayaran Berhasil! Booking Anda di ${propertyName} Telah Dikonfirmasi`);
        console.log("========================================================\n");
        return;
    }
...
export const sendReminderEmail = async (to, bookingData) => {
    const propertyName = bookingData.room_unit?.room_type?.property?.name || "Properti Kami";
    const checkInDate = new Date(bookingData.check_in).toLocaleDateString("id-ID", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
    if (!transporter) {
        console.log("\n📬 ========================================================");
        ...
        console.log(`Tujuan: ${to}`);
        console.log(`Subjek: Pengingat: Besok Waktunya Check-in di ${propertyName}!`);
        console.log("========================================================\n");
        return;
    }
    await transporter.sendMail({
