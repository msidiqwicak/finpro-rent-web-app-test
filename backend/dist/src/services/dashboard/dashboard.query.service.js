...
export const getTenantProfile = async (userId) => {
    ...
    if (!tenant)
        throw new Error("Profil tenant tidak ditemukan.");
    return tenant;
};
export const getAllConfirmedBookings = async (tenantId) => {
...
export const getActiveBookingsCount = async (tenantId, now) => {
    return prisma.booking.count({
        where: { status: "CONFIRMED", check_out: { gte: now }, room_unit: { room_type: { property: { tenant_id: tenantId } } } },
    });
};
export const getUpcomingCheckins = async (tenantId, now) => {
    return prisma.booking.findMany({
