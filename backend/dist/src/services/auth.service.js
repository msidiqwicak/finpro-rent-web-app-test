...
import { generateAccessToken, generateVerificationToken, generateResetToken, verifyResetToken, verifyToken, } from "../utils/jwt.js";
import { sendVerificationEmail, sendResetPasswordEmail, sendEmailChangeVerificationEmail, } from "./email.service.js";
...
export const login = async (email, password, requestedRole) => {
...
const createSocialUser = async (name, email, provider, providerId) => {
    const user = await prisma.users.create({
 ...
 */
export const socialLogin = async (email, name, provider, providerId, action, requestedRole) => {
...
const buildTenantAwareResponse = async (user, name, requestedRole, action) => {
