...
const validateRoomData = (data) => {
    ...
    if (data.capacity !== undefined && Number(data.capacity) > 20)
        throw new Error("Max capacity is 20.");
    if (data.total_units !== undefined && Number(data.total_units) > 200)
        throw new Error("Max total units is 200.");
};
export const createRoomType = async (userId, propertyId, data, files) => {
...
const handleRoomUnits = async (tx, roomTypeId, newUnits, oldUnits) => {
    ...
    if (toCreate.length > 0)
        await tx.room_unit.createMany({ data: toCreate });
};
export const updateRoomType = async (userId, roomTypeId, data, files) => {
    const tenantId = await getTenantId(userId);
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (newTotalUnits !== undefined)
            await handleRoomUnits(tx, roomTypeId, newTotalUnits, existing.total_units ?? 0);
        return updated;
    });
};
export const deleteRoomType = async (userId, roomTypeId) => {
    const tenantId = await getTenantId(userId);
