...
export default function EditRoomTypeModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (existingImages.length + newImages.length > 5) {
      setError(
        `A maximum of 5 photos are allowed (You selected ${existingImages.length} old + ${newImages.length} new).`,
      );
      setLoading(false);
      return;
    }
            ...
            {(existingImages.length > 0 || newImages.length > 0) && (
              <div className="flex gap-3 overflow-x-auto pb-3 hide-scrollbar mb-2">
                ...
                {existingImages.map((url, i) => (
                  <div
                    key={`old-${i}`}
                    className="w-20 h-20 shrink-0 rounded-xl overflow-hidden border border-outline-variant relative group"
                  >
                    <img
                      src={getImageUrl(url)}
                      alt={`Old ${i}`}
                ...
                {newImages.map((file, i) => (
                  <div
                    key={`new-${i}`}
                    className="w-20 h-20 shrink-0 rounded-xl overflow-hidden border-2 border-primary/50 relative group"
                  >
                    <img
