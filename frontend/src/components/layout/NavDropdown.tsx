...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold tracking-wide uppercase px-2.5 py-0.5 rounded-full ${isUser ? "bg-secondary-container text-on-secondary-container" : "bg-primary-fixed text-on-primary-fixed"}`}
    >
      {role}
    </span>
  );
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 px-4 py-3 bg-surface-low">
        <p className="font-body font-bold text-[14px] text-on-surface truncate">
          {user.name}
        </p>
      ...
      {isUser && (
        <>
          <DropdownItem to="/profile" icon="person" label="My Profile" />
          <DropdownItem
            to="/bookings"
            icon="calendar_month"
            label="My Bookings"
