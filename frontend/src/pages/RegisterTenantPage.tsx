...
function AlertMsg({ msg, isError }: { msg: string; isError?: boolean }) {
  if (!msg) return null;
  // Map backend Indonesian error to English
  let displayMsg = msg;
...
function TenantInfoBox() {
  return (
    <div className="bg-[--surface-container] p-4 rounded-xl mb-6 flex gap-3 items-start border border-[--outline-variant]">
...
export default function RegisterTenantPage() {
  ...
  return (
    <div className="min-h-screen bg-[--surface-low] flex items-center justify-center p-5 relative overflow-hidden">
      ...
      <div className="w-full max-w-[540px] bg-white rounded-3xl shadow-[--shadow-raised] border border-[--outline-variant]/30 p-8 sm:p-10 relative z-10">
        <AuthHeader 
          title="List Your Property" 
          subtitle="Join our community of eco-friendly hosts and start earning from your property." 
        />
        ...
        {!success && (
          <form onSubmit={handleSubmit} noValidate>
            ...
            <button type="submit" disabled={loading} className="w-full bg-[--secondary] text-white font-bold text-[15px] py-3.5 rounded-xl flex items-center justify-center gap-2 hover:opacity-90 transition-opacity disabled:opacity-70 mt-2">
