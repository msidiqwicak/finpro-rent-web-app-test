...
function AuthHeader({ mode }: { mode: LoginMode }) {
  return (
    <div className="mb-1">
    ...
    <div className="mb-5 relative">
      ...
      <div className="relative flex items-center">
        ...
        <input
          ...
          className="w-full pl-11 pr-11 py-3.5 bg-[--surface] border border-[--outline-variant] rounded-xl text-[15px] text-[--on-surface] focus:outline-none focus:border-[--primary] focus:ring-1 focus:ring-[--primary] transition-all disabled:opacity-60"
        />
        {togglePass && (
          <button type="button" onClick={togglePass.onClick} className="absolute right-4 text-[--outline] hover:text-[--primary] transition-colors">
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (err: any) { setError(err.message === 'Login gagal. Periksa kembali email dan password Anda.' ? 'Login failed. Please check your email and password.' : err.message); }
  ...
  return (
    <div className="min-h-screen bg-[--surface-low] flex items-center justify-center p-5 relative overflow-hidden">
      ...
      <div className="w-full max-w-[480px] bg-white rounded-3xl shadow-[--shadow-raised] border border-[--outline-variant]/30 p-8 sm:p-10 relative z-10">
        ...
        <div className="flex bg-[--surface-container] p-1.5 rounded-xl mb-8">
          <button onClick={() => { setMode('USER'); setError(''); }} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[14px] font-bold transition-all ${!isTenant ? 'bg-white text-[--primary] shadow-sm' : 'text-[--on-surface-variant] hover:text-[--on-surface]'}`}>
            <span className="material-symbols-outlined text-[18px]">person</span> Guest
          </button>
          <button onClick={() => { setMode('TENANT'); setError(''); }} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[14px] font-bold transition-all ${isTenant ? 'bg-[--secondary] shadow-sm' : 'text-[--on-surface-variant] hover:text-[--on-surface]'}`}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <button type="submit" disabled={loading} className={`w-full text-white font-bold text-[15px] py-3.5 rounded-xl flex items-center justify-center gap-2 hover:opacity-90 transition-all disabled:opacity-70 mt-2 ${isTenant ? 'bg-[--secondary]' : 'bg-[--primary]'}`}>
