      ...
      <main className="bg-surface-low min-h-screen">
        <div className="max-w-[1280px] mx-auto px-5 pt-6 pb-16 md:px-8 lg:px-16">
          ...
          <div className="rounded-2xl overflow-hidden mb-10">
            <div
              className={`grid gap-2 ${allImages.length >= 5 ? "grid-cols-4 grid-rows-2 h-[400px] md:h-[500px]" : allImages.length >= 2 ? "grid-cols-2 h-[300px] md:h-[400px]" : "grid-cols-1 aspect-[16/9] md:aspect-[21/9]"}`}
            >
              {allImages.slice(0, 5).map((url, i) => (
                <div
                  key={i}
                  className={`relative w-full h-full overflow-hidden rounded-xl ${i === 0 && allImages.length >= 5 ? "col-span-2 row-span-2" : ""}`}
                >
                  <img
                    src={url}
                    alt={`${property.name} - ${i + 1}`}
                    className="w-full h-full object-cover object-center hover:scale-105 transition-transform duration-500 bg-surface-container-high"
                  />
                </div>
              ))}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {allAmenities.length > 0 && (
                <div className="mb-8 pb-8 border-b border-outline-variant/50">
                  <h2 className="font-display font-bold text-[20px] text-on-surface mb-4">
                    Amenities
                  </h2>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {allAmenities.map((a) => (
                      <div
                        key={a}
                        className="flex items-center gap-2.5 text-[14px] text-on-surface-variant py-2"
                      >
                  ...
                  {property.room_type.map((rt) => (
                    ...
                    >
                      <div className="w-24 h-20 rounded-lg overflow-hidden shrink-0 bg-surface-container-high">
                        <img
                          src={rt.image_urls[0] ?? PLACEHOLDER}
                          alt={rt.name}
                          className="w-full h-full object-cover"
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-display font-bold text-[16px] text-on-surface mb-1">
                          {rt.name}
                        </p>
                        <p className="text-[13px] text-on-surface-variant mb-2">
                          Up to {rt.capacity} guest{rt.capacity > 1 ? "s" : ""}{" "}
                          · {rt.total_units} unit{rt.total_units > 1 ? "s" : ""}
                        </p>
                        <p className="font-display font-extrabold text-[18px] text-primary-container">
                          Rp {formatPrice(rt.adjusted_price)}
                          <span className="text-[13px] font-normal text-on-surface-variant font-body">
                            {" "}
                            / night
                          </span>
                        </p>
                      ...
                      {selectedRoom?.id === rt.id && (
                        <span className="material-symbols-outlined text-primary text-[24px] self-center [font-variation-settings:'FILL'_1]">
                          check_circle
                        </span>
                      )}
                    </button>
                  ...
                  {selectedRoom && (
                    <div className="mb-5">
                      <p className="font-display font-extrabold text-[26px] text-primary-container leading-tight">
                        ...
                        <span className="text-[14px] font-normal text-on-surface-variant font-body">
                          {" "}
                          / night
                        </span>
                      </p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          disabled={guests <= 1}
                          className="w-8 h-8 rounded-full border border-outline-variant bg-transparent text-primary flex items-center justify-center cursor-pointer disabled:opacity-30"
                        >
                          −
                        </button>
                        <button
                    ...
                    <div className="mt-5 pt-5 border-t border-outline-variant/50 space-y-2">
                      <div className="flex justify-between text-[14px] text-on-surface-variant">
                        ...
                        <span>
                          Rp{" "}
                          {formatPrice(
                            selectedRoom.adjusted_price * nightCount,
                          )}
                        </span>
                      </div>
                      <div className="flex justify-between text-[14px] text-on-surface-variant">
                        ...
                        <span>
                          ...
                          {formatPrice(
                            Math.round(
                              selectedRoom.adjusted_price * nightCount * 0.05,
                            ),
                          )}
                        </span>
                      </div>
                      <div className="flex justify-between text-[16px] font-bold text-on-surface pt-3 border-t border-outline-variant/50">
                        ...
                        <span>
                          ...
                          {formatPrice(
                            Math.round(
                              selectedRoom.adjusted_price * nightCount * 1.05,
                            ),
                          )}
                        </span>
                      </div>
              ...
              <div className="fixed bottom-0 left-0 right-0 z-50 bg-surface-white border-t border-outline-variant shadow-[0_-4px_20px_rgba(0,0,0,0.08)] px-5 py-3 flex items-center justify-between lg:hidden">
                <div>
                  {selectedRoom && (
                    <p className="font-display font-extrabold text-[18px] text-primary-container">
                      Rp {formatPrice(selectedRoom.adjusted_price)}
                      <span className="text-[12px] font-normal text-on-surface-variant font-body">
                        {" "}
                        / night
                      </span>
                    </p>
  ...
  return (
    <main className="bg-surface-low min-h-screen">
      <div className="max-w-[1280px] mx-auto px-5 pt-6 pb-16 md:px-8 lg:px-16 animate-pulse">
        ...
        <div
          className="grid grid-cols-4 grid-rows-2 gap-1 rounded-2xl overflow-hidden mb-10"
          style={{ height: 460 }}
        >
...
function ErrorState({ message }: { message: string | null }) {
  return (
    <main className="bg-surface-low min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-on-surface-variant text-[16px] max-w-sm text-center">
        {message ?? "Something went wrong."}
      </p>
      <Link
        to="/explore"
        className="px-6 py-2.5 rounded-full bg-primary text-on-primary text-[13px] font-bold"
      >
        Back to Explore
      </Link>
    </main>
